...
import com.org.kulture.kulture.model.Sport_GridView_ImageAdapter;
import com.org.kulture.kulture.model.Sport_Wear_Model;
...
public class SportItemsList extends AppCompatActivity {
    ImageView back_from_productdetails,product_cart;
    String title,ids,name;
    TextView cart_item;
    LinearLayout just_click;
    ...
    private Sport_GridView_ImageAdapter sport_gridView_imageAdapter;
    private ArrayList<Sport_Wear_Model> sport_wear_models;
    String img_url,img_ ;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_sport_items_list);
        ...
        just_click=(LinearLayout)findViewById(R.id.just_click);
        category_name=(TextView)findViewById(R.id.category_name_sport);
            category_name.setText("Sports Wear");
        ...
        back_from_productdetails.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                Intent intent=new Intent(SportItemsList.this,Homescreen.class);
                startActivity(intent);
                SportItemsList.this.finish();
            }
        });
        ...
        product_cart.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                Intent intent=new Intent(SportItemsList.this,CartActivity.class);
                startActivity(intent);
                SportItemsList.this.finish();
            }
        });
        ...
        load_corporate_list = (ExpandableHeightGridView) findViewById(R.id.load_sport_wear_list);
        load_corporate_list.setExpanded(true);
        if (isInternetPresent) {
            sport_wear_models = new ArrayList<Sport_Wear_Model>();
            sport_gridView_imageAdapter = new Sport_GridView_ImageAdapter(this, R.layout.sports_wearlist, sport_wear_models);
            load_corporate_list.setAdapter(sport_gridView_imageAdapter);
            //getting the progressbar
            progressBar = (ProgressBar) findViewById(R.id.progressBar);
            //making the progressbar visible
            progressBar.setVisibility(View.VISIBLE);
            new Sport_Wear_Data().execute();
            ...
            load_corporate_list.setOnItemClickListener(new AdapterView.OnItemClickListener() {
                public void onItemClick(AdapterView<?> parent, View v, int position, long id) {
                    Sport_Wear_Model item = (Sport_Wear_Model) parent.getItemAtPosition(position);
                    Intent intent = new Intent(SportItemsList.this, Inquiry_Form.class);
                    intent.putExtra("id", String.valueOf(item.getSport_id()));
                    intent.putExtra("title", item.getSport_name());
                    intent.putExtra("image", item.getSport_image());
                    intent.putExtra("name", name);
                    intent.putExtra("ids",ids);
                    intent.putExtra("slug",title);
                    intent.putExtra("imgarray",item.getSport_image2());
                    intent.putExtra("findsession","SportItemsList");
                    startActivity(intent);
                    SportItemsList.this.finish();
                }
            });
    ...
    //--------------------------------------Active List ----------------------------
    private class Sport_Wear_Data extends AsyncTask<String, String, Boolean> {
                ...
                try {
                    ...
                    for (int i = 0; i < array.length(); i++) {
                        ...
                        for(int k=0; k < corporate.length(); k++) {
                            JSONObject corporateobject = corporate.getJSONObject(1);
                           img_ = corporateobject.getString("src");
                        }
                            Sport_Wear_Model sport_wear_model = new Sport_Wear_Model();
                            sport_wear_model.setSport_id(id);
                            sport_wear_model.setSport_name(title);
                            sport_wear_model.setSport_image(img_url);
                            sport_wear_model.setSport_image2(img_);
                            sport_wear_models.add(sport_wear_model);
        ...
        protected void onPostExecute(Boolean aBoolean) {
            sport_gridView_imageAdapter.notifyDataSetChanged();
